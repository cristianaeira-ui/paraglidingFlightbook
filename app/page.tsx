...
interface Flight {
  id: number; datum: string; startplatz: string; landeplatz: string
  gleitschirm: string; gurtzeug: string | null; flugzeit: number | null
  bemerkungen: string | null; created_at: string
}
interface Options { startplaetze: string[]; landplaetze: string[]; gleitschirme: string[]; gurtzeugs: string[] }
interface FormState {
  datum: string; startplatz: string; landeplatz: string; gleitschirm: string
  gurtzeug: string; flugzeitH: string; flugzeitM: string; bemerkungen: string
}
...
const DEFAULT_FORM: FormState = { datum: todayStr(), startplatz:'', landeplatz:'', gleitschirm:'', gurtzeug:'', flugzeitH:'', flugzeitM:'', bemerkungen:'' }

function formatDate(s: string) {
  if (!s) return '—'
  const [y,m,d] = s.split('-')
  return new Date(Number(y),Number(m)-1,Number(d)).toLocaleDateString('de-CH',{weekday:'short',day:'numeric',month:'short',year:'numeric'})
}
...
function ComboBox({value,onChange,options,placeholder,id}:{value:string;onChange:(v:string)=>void;options:string[];placeholder:string;id?:string}) {
  const [open,setOpen]=useState(false)
  const ref=useRef<HTMLInputElement>(null)
  const filtered=value.trim()?options.filter(o=>o.toLowerCase().includes(value.toLowerCase().trim())):options
  const hasExact=options.some(o=>o.toLowerCase()===value.trim().toLowerCase())
  const showAdd=value.trim().length>0&&!hasExact
  const select=(val:string)=>{onChange(val);setOpen(false);ref.current?.blur()}
  return (
    <div className="combo-wrap">
      ...
      {open&&(filtered.length>0||showAdd)&&(
        <div className="combo-dropdown">
          ...
          {showAdd&&<div className="combo-item combo-add" onPointerDown={e=>{e.preventDefault();select(value.trim())}}>+ "{value.trim()}" hinzufügen</div>}
        </div>
      )}
    </div>
  )
...
export default function App() {
  ...
  async function saveFlight() {
    ...
    const body={datum:form.datum,startplatz:form.startplatz.trim(),landeplatz:form.landeplatz.trim(),gleitschirm:form.gleitschirm.trim(),gurtzeug:form.gurtzeug.trim()||null,flugzeit,bemerkungen:form.bemerkungen.trim()||null}
    try {
    ...
    <>
      <header className="app-header">
        <div className="header-inner">
          <div className="header-greeting">Willkommen zurück, Cristiana 👋</div>
              ...
              {loading?(
                ...
                <div key={f.id} className="flight-item" onClick={()=>setSelected(f)}>
                  ...
                  <div className="flight-chips">
                    ...
                    {f.gurtzeug?<span className="chip">🎽 {f.gurtzeug}</span>:null}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="card">
              ...
              <div className="stat-grid">
                <div className="stat-tile blue"><div className="stat-tile-value">{dashFlights.length}</div><div className="stat-tile-label">Flüge {dashYear!=='all'?dashYear:'total'}</div></div>
                <div className="stat-tile blue"><div className="stat-tile-value">{dashH}h {dashM}m</div><div className="stat-tile-label">Gesamtflugzeit</div></div>
